...
本文中主要研究MYSQL在initialize如何创建与UNDO LOG相关的文件和对象，同时启动时又是如何从文件中加载这些对象。‌

UNDO表空间主要管辖回滚段（Rollback Segment），回滚段内再组织成为UNDO LOG、UNDO PAGE等。因而，UNDO表空间的创建主要便是构建回滚段。‌
...
void trx_sysf_create(mtr_t *mtr)
{
...
根据代码描述，为了兼容性考虑，第一个回滚段在创建trx sys page时一并创建并存放于系统表空间。接下来在完成double write buffer 初始化后会继续创建剩余的回滚段，分别为临时表空间和undo表空间内创建。

需要说明的是，创建新的回滚段不仅仅在数据库初始化时存在，也有可能发生在数据库重启时，因为我们有可能会调整回滚段数量这个参数。
...
page_no_t trx_rseg_header_create(...)
{
  // 分配回滚段空间
...
‌创建回滚段的主要任务是创建并初始化回滚段的header page。而主要是为header page初始化undo log slot数组为0，表示该slot尚未分配。以后从回滚段分配undo log时，便是寻找header page中第一个为0的slot。
